...
if [ -f "$WORDLIST" ]; then
    run_with_timer "gobuster dir -u \"$TARGET\" -w \"$WORDLIST\"" "$RESULT_DIR/gobuster.txt" "${RED}--- Gobuster Results ---" "Brute-forcing directories with Gobuster."
else
    echo -e "${RED}[!] Wordlist not found at $WORDLIST. Skipping Gobuster scan.${RESET}"
    echo "Gobuster skipped: Wordlist missing" > "$RESULT_DIR/gobuster.txt"
fi
